<?php
// Stripe Checkout Rylax Gate
// This file is meant to be included, not accessed directly

// Only process if called via include
if (basename($_SERVER['PHP_SELF']) === 'stripe_checkout_rylax.php' && $_SERVER['REQUEST_METHOD'] === 'POST') {
...
if ($httpCode !== 200) {
    echo json_encode(['success' => false, 'error' => 'Failed to fetch checkout data from Rylax']);
    return;
}
...
if (isset($initData['customer'])) {
    ...
    $customerName = $initData['customer']['name'] ?? 'Test User';
...
if (isset($confirmData['payment_intent']['status'])) {
    ...
    if ($status === 'succeeded') {
        ...
        echo json_encode([
            'success' => true,
            'status' => 'approved',
            'message' => '⚠️ 3D Secure required (LIVE card)',
            'checkout_info' => $checkoutInfo
        ]);
        return;
    } else {
...
if (isset($confirmData['error'])) {
    ...
    $liveIndicators = [
        'cvv2_failure', 'invalid_security_code', 'invalid_cvc', 'invalid_cvv',
        'invalid_billing_address', 'insufficient_funds', 'do_not_honor'
    ];
