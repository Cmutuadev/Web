...
$pageTitle = "Razorpay Checker";
$user = $_SESSION['user'];
$credits = getUserCredits();
$isAdmin = isAdmin();
$userPlan = $user['plan'] ?? 'basic';

// Get Razorpay gate from database
$gates = loadGates();
$razorpayGate = $gates['razorpay'] ?? null;
$requiredPlan = $razorpayGate['required_plan'] ?? 'basic';
...
$creditCost = $razorpayGate['credit_cost'] ?? 1;

// Check if Telegram hits are enabled
$settings = loadSettings();
$telegramHitsEnabled = ($settings['telegram_hits_enabled'] ?? 'false') === 'true';

// Default Razorpay sites (Auto Mode)
$defaultSites = [
    'https://razorpay.me/@azhimukham',
    'https://razorpay.me/@merchant1',
    'https://razorpay.me/@merchant2',
    'https://razorpay.me/@store3',
    'https://razorpay.me/@shop4'
];
?>
<!DOCTYPE html>
<html lang="en">
...
<body data-theme="dark">
    <div class="gate-loader" id="gateLoader" style="display: none;">
        ...
        <div class="loader-text" id="loaderText">Loading Razorpay Checker...</div>
    ...
    <main class="main" id="main">
        <div class="container">
            <div class="page-header">
                <h1 class="page-title"><i class="fas fa-rupee-sign"></i> <?php echo $pageTitle; ?></h1>
                <p class="page-subtitle">Test cards against Razorpay payment links | Auto & Self modes</p>
            </div>
            ...
            <div class="features-panel">
                <div class="features-grid">
                    ...
                    <div class="feature-card" onclick="showSiteManager()">
                        ...
                        <div class="feature-desc">Manage Razorpay links</div>
                    </div>
            ...
            <div class="batch-section" id="sitePanel" style="display:none;">
                <div class="batch-header">
                    <div><i class="fas fa-store"></i> <strong>Site Manager (Self Mode Razorpay Links)</strong></div>
                ...
                <div class="add-site-form">
                    <input type="text" id="newSiteUrl" placeholder="https://razorpay.me/@merchant">
            ...
            <div class="checker-card">
                <div style="margin-bottom: 1rem;">
                    <div class="site-mode-buttons">
                        <div class="mode-btn active" id="autoModeBtn" onclick="setMode('auto')">
                            <i class="fas fa-robot"></i> Auto Mode (Built-in Razorpay Links)
                        </div>
                        <div class="mode-btn" id="selfModeBtn" onclick="setMode('self')">
                            <i class="fas fa-user"></i> Self Mode (Your Razorpay Links)
                        </div>
                    </div>
                    <div id="autoSitesDisplay" style="font-size:0.65rem; color:var(--text-muted);">
                        <i class="fas fa-globe"></i> Using built-in Razorpay payment links
                    </div>
                    <div id="selfSitesDisplay" style="display:none; font-size:0.65rem; color:var(--text-muted);">
                        <i class="fas fa-store"></i> Using custom Razorpay links. <a href="#" onclick="showSiteManager(); return false;">Manage Sites</a>
                    </div>
                </div>
                ...
                <div style="margin-bottom: 1rem;">
                    <label style="font-size:0.7rem; color:var(--text-muted);"><i class="fas fa-rupee-sign"></i> Amount (INR)</label>
                    <input type="number" id="amount" class="form-control" value="1" min="1" max="10000" style="width:120px;">
                </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            showLoader('Loading Razorpay Checker...', 800);
            loadCustomSites();
            loadProxies();
            document.getElementById('amount').value = localStorage.getItem('razorpay_amount') || '1';
        });
        ...
        function loadCustomSites() {
            const saved = localStorage.getItem('razorpay_custom_sites');
            if (saved) customSites = JSON.parse(saved);
            renderCustomSites();
        }
        
        function saveCustomSites() {
            localStorage.setItem('razorpay_custom_sites', JSON.stringify(customSites));
            renderCustomSites();
        }
        ...
        function renderCustomSites() {
            ...
            if (customSites.length === 0) {
                container.innerHTML = '<div class="site-item"><div class="site-url">No custom Razorpay links added yet</div></div>';
                return;
            }
            let html = '';
        ...
        function addCustomSite() {
            ...
            if (!url) return Swal.fire('Error', 'Please enter a Razorpay link URL', 'error');
            if (!url.startsWith('http')) return Swal.fire('Error', 'URL must start with http:// or https://', 'error');
            if (!url.includes('razorpay.me')) {
                return Swal.fire('Warning', 'Make sure this is a valid Razorpay payment link', 'warning');
            }
            customSites.push(url);
            saveCustomSites();
            document.getElementById('newSiteUrl').value = '';
            Swal.fire('Added', 'Razorpay link added successfully', 'success');
        }
        ...
        function loadProxies() {
            const saved = localStorage.getItem('razorpay_proxies');
            if (saved) proxies = JSON.parse(saved);
            renderProxies();
        }
        
        function saveProxies() {
            localStorage.setItem('razorpay_proxies', JSON.stringify(proxies));
            renderProxies();
        }
        ...
        function downloadAllResults() {
            ...
            a.download = `razorpay_results_${new Date().toISOString().slice(0,19)}.txt`;
            a.click();
        ...
        function downloadApprovedCards() {
            ...
            const a = document.createElement('a');
            a.href = URL.createObjectURL(blob);
            a.download = `razorpay_approved_${new Date().toISOString().slice(0,19)}.txt`;
            a.click();
            URL.revokeObjectURL(a.href);
        }
        ...
        function exportJSON() {
            if (results.length === 0) return;
            const exportData = { timestamp: new Date().toISOString(), gateway: 'razorpay', mode: currentMode, total: results.length, results: results };
            const blob = new Blob([JSON.stringify(exportData, null, 2)], { type: 'application/json' });
            const a = document.createElement('a');
            a.href = URL.createObjectURL(blob);
            a.download = `razorpay_results_${new Date().toISOString().slice(0,19)}.json`;
            a.click();
            URL.revokeObjectURL(a.href);
        }
        ...
        function addResult(card, status, message, site, amount) {
            ...
            const html = `<div class="result-item ${statusClass}" data-status="${status}">
                ...
                ${site ? `<div class="result-details"><i class="fas fa-store"></i> Site: ${escapeHtml(site)} | 💰 ₹${amount}</div>` : ''}
            ...
            results.unshift({ card, status, message, time: new Date().toLocaleTimeString(), site, amount });
            updateStats();
        ...
        async function processCards(isMass = false) {
            ...
            localStorage.setItem('razorpay_amount', amount);
            ...
            for (let i = 0; i < cards.length && !shouldStop; i++) {
                ...
                try {
                    let fetchUrl = '/gate/razorpay.php?cc=' + encodeURIComponent(card) + '&site=' + encodeURIComponent(site) + '&amount=' + amount;
                    ...
                    if (responseText.includes('APPROVED') || responseText.includes('Payment successful')) {
                        status = 'approved';
                    ...
                    addResult(card, status, message, site, amount);
                    
                } catch(err) {
                    addResult(card, 'declined', 'Request failed: ' + err.message, site, amount);
                }
        ...
        document.getElementById('sitesFileInput').addEventListener('change', function(e) {
            ...
            reader.onload = (e) => {
                ...
                saveCustomSites();
                Swal.fire('Loaded', `${sites.length} Razorpay links added`, 'success');
            };
            reader.readAsText(file);
        });
