    ...
    <main class="main" id="main">
        <div class="container">
            <div class="page-header">
                ...
                <p class="page-subtitle">Card authentication checker</p>
            </div>
    ...
    <script>
        ...
        function addResult(card, status, message, binInfo) {
            ...
            const resultHtml = `<div class="result-item" data-status="${status}">
                ...
                <div class="result-content">
                    ...
                    <div class="result-status">${status.toUpperCase()} - ${escapeHtml(message)}</div>
        ...
        async function updateDashboardStats(card, status, message) {
            try {
                await fetch('/api/update-stats.php', {
                    ...
                    body: JSON.stringify({ card: card, status: status, reason: message, gateway: 'auth' })
                });
            } catch(e) {}
        }
        ...
        async function processCards() {
            ...
            for (let i = 0; i < cards.length && !shouldStop; i++) {
                ...
                try {
                    ...
                    const response = await fetch('/gate/auth_checker.php', { method: 'POST', body: formData });
            ...
            Swal.fire({ title: 'Complete!', text: 'Processed ' + cards.length + ' cards', icon: 'success', toast: true, timer: 3000 });
        }
        ...
        function downloadResults(type) {
            ...
            if (text) {
                ...
                a.href = url;
                a.download = `auth-${type}-${new Date().toISOString().slice(0,19)}.txt`;
                a.click();
